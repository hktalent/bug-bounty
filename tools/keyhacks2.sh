...
echo    " 46. IP Stack Key 			47. Facebook AppID and Secret		    48. Loqate API key	"
echo	" 49. Twillio account  			50. "
echo ""
...
case $number in
	...
	19)	
		...
		read key
		echo "POC 1: List systems"
		echo $line
		curl -H "x-api-key: $key" "https://console.jumpcloud.com/api/systems"
		echo ""
		echo "POC 2: List system users"
		echo $line
		curl -H "x-api-key: $key" "https://console.jumpcloud.com/api/systemusers"
		echo ""
		echo "POC 3: If SSO Applications AWS login SAM config is presents. This would leads to AWS account takeover"
		echo $line
		curl -H "x-api-key: $key" "https://console.jumpcloud.com/api/applications"
		echo ""
		echo "**If you furhter want to execute system command refer:  https://docs.jumpcloud.com/1.0/commands/create-a-command"
		echo ""
		;;
	...
	45)
		...
		bearer=$(curl -s https://api.sandbox.paypal.com/v1/oauth2/token -H "Accept: application/json" -H "Accept-Language: en_US" -u "$client:$secret" -d "grant_type=client_credentials")
	...
	47)
		echo -n "Enter Facebook ID: "
		read id
		echo -n "Enter Facebook Secret: "
		read secret
		echo " NOT IMPLEMENTED! " $id+$secret
		;;
		
	48)
		echo -n "Enter Loqate api key : "
		read key
		echo ""
		curl -s "http://api.addressy.com/Capture/Interactive/Find/v1.00/json3.ws?Key="+$key+"&Countries=US,CA&Language=en&Limit=5&Text=BHAR"
		echo ""
		;;
	...
	49)
		...
		read token
		curl -X GET 'https://api.twilio.com/2010-04-01/Accounts.json' -u $sid:$token
		echo ""
		;;
